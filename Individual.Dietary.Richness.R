...
input<-data[!duplicated(data$sample), ]
input<-dplyr::select(input, sample, month.cat, age.cat, sex, ring)
input$richness.otu <- NA
input$richness.fam <- NA

for (i in 1:nrow(input)){
  
  temp<-data[which(data$sample == input$sample[i]),]
  input$richness.otu[i]<-length(unique(temp$otu.id))
  rm(temp)
}
...
p.model.otu <- glm(richness.otu ~ month.cat + age.cat + sex, family = 'poisson', data = input)
...
nb.model.otu <- glm.nb(richness.otu ~ month.cat + age.cat + sex, data = input)
...
pchisq(2 * (logLik(nb.model.otu) - logLik(p.model.otu)), df = 1, lower.tail = FALSE) # nb gives better fit
...
p.model.fam <- glm(richness.fam ~ month.cat + age.cat + sex, family = 'poisson', data = input)
...
nb.model.fam <- glm.nb(richness.fam ~ month.cat + age.cat + sex, data = input)
...
pchisq(2 * (logLik(nb.model.fam) - logLik(p.model.fam)), df = 1, lower.tail = FALSE) # nb gives better fit
